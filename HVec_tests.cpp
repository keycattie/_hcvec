...
main()
{
  ...
  ASSERT(fabsf(vec3a.vlen() - 3.742f) < 0.001);
  auto vec3b = HVec<3>{ 0.0f, 0.0f, 1.0f };
  ASSERT(fabsf(vec3b.vlen() - 1.0f) < 0.00001);
  TESTEND;
  ...
  vec3b = HVec<3>{ .0f, .0f, .0f };
  ASSERT(!(vec3a < vec3b));
  vec3a = HVec<3>{ .1f, -.2f, -.3f };
  vec3b = HVec<3>{ .1f, .1f, .0f };
  ASSERT(vec3a > vec3b);
  vec3a = HVec<3>{ .1f, .2f, .3f };
  vec3b = HVec<3>{ .0f, .0f, .3f };
  auto vec3c = HVec<3>{ .0f, .0f, .3f };
  ASSERT(vec3a >= vec3b);
  ASSERT(vec3c >= vec3b);
  vec3c = HVec<3>{ .1f, .2f, .3f };
  ASSERT(vec3b <= vec3a);
  ASSERT(vec3b <= vec3a);
  TESTEND;
  ...
  auto vec3c = HVec<3>{ .3f, .2f, .9f };
  vec3a = vec3a.sum(vec3b);
  // std::cout << vec3a.to_string() << std::endl;
  ASSERT(vec3c.eq(vec3a));
  TESTEND;
  TEST("mul *f *=f - -= -@", true);
  auto vec3a = HVec<3>{ .1f, -.2f, .3f };
  auto vec3b = HVec<3>{ .2f, -.4f, .6f };
  auto vec3c = vec3a.mul(2.0f);
  ASSERT(vec3b.eq(vec3c));
  vec3c = vec3a * 2.0f;
  ASSERT(vec3b.eq(vec3c));
  vec3c *= 2.0f;
  ASSERT(vec3b.eq(vec3c));
  vec3c = vec3b - vec3a;
  ASSERT(vec3c.eq(HVec<3>{ .1f, -.2f, .3f }));
  vec3c = vec3b;
  vec3c -= vec3a;
  ASSERT(vec3c.eq(vec3c));
  vec3c = -vec3a;
  ASSERT(vec3c.eq(HVec<3>{ -.1f, .2f, -.3f }));
  TESTEND;
  TEST("norm !", true);
  ...
  vec3a = vec3a.norm();
  ASSERT(vec3b.eq(vec3a));
  vec3a = HVec<3>{ .1f, -.2f, .5f };
  vec3a = !vec3a;
  ASSERT(vec3b.eq(vec3a));
  TESTEND;
  TEST("dprod * *=", true);
  auto vec3a = HVec<3>{ .1f, -.2f, .5f };
  auto vec3b = HVec<3>{ .2f, -.4f, 1.0f };
  ASSERT(vec3a.dprod(vec3b) - .6f < 0.0001);
  TESTEND;
  TEST("cprod3", true);
  auto vec3a = HVec<3>{ .1f, .2f, .3f };
